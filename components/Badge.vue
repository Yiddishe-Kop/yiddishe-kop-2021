<template>
  <span :class="getClasses().badge" class="inline-flex items-center font-medium rounded-full">
    <svg v-if="dot" :class="getClasses().dot" class="-ms-0.5 me-1.5 h-2 w-2" fill="currentColor" viewBox="0 0 8 8">
...
export default {
  ...
  methods: {
    getClasses() {
      ...
      switch (this.size) {
        case 'sm':
          badgeClasses += ' px-1 text-xs leading-4';
          break;
        case 'base':
          badgeClasses += ' px-2.5 py-0.5 text-sm leading-4';
          break;
        case 'lg':
          badgeClasses += ' px-3 py-0.5 leading-5';
          break;
        case 'xl':
          badgeClasses += ' px-4 py-1 font-bold text-lg leading-5';
          break;
      }
      if (this.uppercase) {
